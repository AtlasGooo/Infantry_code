...
public:
	...
	*/
	void solveAngles( Mat& tvec,Mat& rvec );
	...
	* @param inputPoint centerPoint the center point of target armor
	* @param input type the type of armor BIG_ARMOR or SMALL_ARMOR
	* @param output x   '<-' left is minus-       '->' right is positive+
	* @param output y   up is negative; down is positive
	* @param output z   always positive unit is mm
	*/
	void getPos(vector<Point2f> & contourPoints, Point2f centerPoint, ArmorType type, Mat& tvec,Mat& rvec );
