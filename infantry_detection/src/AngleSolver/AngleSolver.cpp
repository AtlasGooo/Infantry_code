...
void AngleSolver::solveAngles( Mat& tvec,Mat& rvec )
{
	Mat _rvec;
	// printf("camera_matrix size: (%d,%d)\n",CAMERA_MATRIX.size[0],CAMERA_MATRIX.size[1]);
	// cout<<CAMERA_MATRIX<<endl;
	// printf("DISTORTION_COEFF size: (%d,%d)\n",DISTORTION_COEFF.size[0],DISTORTION_COEFF.size[1]);
	// cout<<DISTORTION_COEFF<<endl;
	// printf("SMALL_ARMOR_POINTS_3D size: (%d)\n",SMALL_ARMOR_POINTS_3D.size());
	// cout<<SMALL_ARMOR_POINTS_3D<<endl;
	// printf("BIG_ARMOR_POINTS_3D size: (%d)\n",BIG_ARMOR_POINTS_3D.size());
	// cout<<BIG_ARMOR_POINTS_3D<<endl;
	// printf("targetContour size: (%d)\n",targetContour.size());
...
void AngleSolver::getPos(vector<Point2f>& contourPoints,Point2f cenerPoint, ArmorType type, Mat& tvec,Mat& rvec )
{
	setTarget(contourPoints, cenerPoint, type);
	solveAngles(tvec,rvec);
    // compensateAngle();
}
