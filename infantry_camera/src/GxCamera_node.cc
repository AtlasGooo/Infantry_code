...
int main(int argc, char** argv){
    ...
    ros::Publisher cam_info_pub = nh.advertise<CameraInfo>("camera_info", 10);;
    ...
    nh.getParam("/"+camera_name+"/gxcam_height", height);
    nh.getParam("/"+camera_name+"/gxcam_width",  width);

    vector<double> intrinsics;
    nh.getParam("/"+camera_name+"/gxcam_intrinsics", intrinsics);
    boost::array<double, 9> intrinsics_array = IntrinsicsToBoostArray(intrinsics);

    vector<double> distortion;
    nh.getParam("/"+camera_name+"/gxcam_distortion", distortion);
    ROS_INFO_STREAM("Load camera distortion params: " << endl << DistortionToString(distortion));

    string distortion_model;
    nh.getParam("/"+camera_name+"/gxcam_distortion_model", distortion_model);
    ROS_INFO_STREAM("Load camera distortion model: " << distortion_model);

    // Load camera setting parameters
    double exp_time = 0.0;
    double frame_rate = 0.0;
    nh.getParam("/"+camera_name+"/gxcam_expsure_time", exp_time);
    nh.getParam("/"+camera_name+"/gxcam_frame_rate", frame_rate);
